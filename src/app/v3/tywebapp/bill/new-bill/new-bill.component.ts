...
export class NewBillComponent {
  ...
  submitData(): void {
    ...
    const payload: submit_bill = {
      ...
      bill_subtotal: parseFloat((Number(this.bill.bill_subtotal) / 100).toFixed(2)),
      bill_tax: parseFloat((Number(this.bill.bill_tax) / 100).toFixed(2)),
      bill_tips: parseFloat((Number(this.bill.bill_tips) / 100).toFixed(2)),
      bill_payer_id: this.bill.bill_payer?.id || '',
      paid_wallet_id: this.bill.paid_wallet?.id || '',
      paid_amount: parseFloat((Number(this.bill.paid_amount) / 100).toFixed(2)),
      ...
      bill_items: this.bill_items.map((item) => ({
        ...
        price: parseFloat((Number(item.price) / 100).toFixed(2)),
        tax: parseFloat((Number(item.tax) / 100).toFixed(2)),
